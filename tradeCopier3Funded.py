...
SIRIX_MASTER_USER_ID: str = "263280"    # 263269
...
VOLUME_MODE: str = "risk_master_multiple"  # use K × master's % risk
RISK_MULTIPLIER: float = 1.0              # this will be overridden per follower

# (Other sizing modes kept available for completeness; not used unless you switch VOLUME_MODE)
FIXED_LOTS: float = 0.10
LOT_MULTIPLIER: float = 1.0
PER_TRADE_RISK_PCT: float = 0.10  # only used if switch VOLUME_MODE="risk_percent"
...
# ---- Files ----
STATE_PATH: str = "copier_state-263269.json"
...
RISK_PREVIEW_LIMIT: int = 15                # how many positions to show in the preview
...
DEBUG_ATTACH_MT5_IN_DRY_RUN: bool = False
# Use real tick_value/tick_size for risk preview even in dry-run (if MT5 attached)
DEBUG_USE_REAL_TICK_IN_DRY_RUN: bool = False


# ============================================================
# MULTI-FOLLOWER CONFIG (3 funded accounts)
# ============================================================

# Each follower is one MT5 account the copier will control.
# login/server/password/terminal_path for 3 demo/funded accounts.
FOLLOWERS: Dict[str, Dict[str, Any]] = {
    "FUND_5K": {
        "terminal_path": r"C:\MT5\TradeCopier-263269\FUND_5K\terminal64.exe",
        "login": 52611389,
        "password": "z!nrdnlO3t6kOm",
        "server": "ICMarketsSC-Demo",

        # Per-follower risk and safety
        # K × master's % risk
        "risk_multiplier": 1.0,        # 1.0 = same % risk as master for 5k
        "daily_loss_pct": 0.04,        # e.g. 4% daily max (adjust to your prop rule)

        # Separate magic + state file so accounts never clash
        "magic_number": 86543001,
        "state_path": "copier_state_5k.json",
    },
    "FUND_10K": {
        "terminal_path": r"C:\MT5\TradeCopier-263269\FUND_10K\terminal64.exe",
        "login": 52611391,
        "password": "D&KvS$2KPBrYso",
        "server": "ICMarketsSC-Demo",

        "risk_multiplier": 1.0,        # same % risk as master for 10k
        "daily_loss_pct": 0.04,        # adjust to your rules
        "magic_number": 86543002,
        "state_path": "copier_state_10k.json",
    },
    "FUND_25K": {
        "terminal_path": r"C:\MT5\TradeCopier-263269\FUND_25K\terminal64.exe",
        "login": 52611395,
        "password": "Hk30ZH&z$mNf9u",
        "server": "ICMarketsSC-Demo",

        "risk_multiplier": 1.0,        # same % risk as master for 25k
        "daily_loss_pct": 0.04,        # adjust to your rules
        "magic_number": 86543003,
        "state_path": "copier_state_25k.json",
    },
}

# Per-follower runtime state (kept separate for each account)
FOLLOWER_STATE: Dict[str, Dict[str, Any]] = {
    name: {
        "TRADING_HALTED": False,
        "SESSION_DAY_KEY": None,
        "DAY_START_EQUITY": None,
        "preview_printed": False,
        "link_map": {},        # type: Dict[Any, FollowerLink]
        "links_loaded": False, # have we re-linked existing positions yet?
    }
...
def init_mt5_for_current_globals() -> bool:
    """
    Helper for multi-follower mode:
    - Shutdown any existing MT5 connection.
    - Re-init using *current* global MT5_* settings.
    """
    if not DRY_RUN or DEBUG_ATTACH_MT5_IN_DRY_RUN:
        try:
            mt5.shutdown()
        except Exception:
            pass
...
def apply_master_to_follower(follower_name: str,
                             cfg: Dict[str, Any],
                             master_snap: Dict[Any, MasterPosition]) -> None:
    """
    Apply the master snapshot to ONE follower:
    - Swap in that follower's MT5 config and risk/safety config.
    - Re-attach to the follower's MT5 terminal.
    - Ensure daily state & link map.
    - Call process_master_changes() using this follower's runtime.
    """
    global MT5_LOGIN, MT5_PASSWORD, MT5_SERVER, MT5_TERMINAL_PATH
    global MAGIC_NUMBER, STATE_PATH, DAILY_LOSS_LIMIT_PCT, RISK_MULTIPLIER
    global TRADING_HALTED, SESSION_DAY_KEY, DAY_START_EQUITY, preview_printed, link_map

    st = FOLLOWER_STATE[follower_name]

    # 1) Set config globals for THIS follower
    MT5_LOGIN = cfg["login"]
    MT5_PASSWORD = cfg["password"]
    MT5_SERVER = cfg["server"]
    MT5_TERMINAL_PATH = cfg["terminal_path"]
    ...
    # 2) Restore this follower's runtime globals
    TRADING_HALTED = st["TRADING_HALTED"]
    SESSION_DAY_KEY = st["SESSION_DAY_KEY"]
    DAY_START_EQUITY = st["DAY_START_EQUITY"]
    preview_printed = st["preview_printed"]
    link_map = st["link_map"]

    # 3) Attach to this follower's MT5 terminal
    if not init_mt5_for_current_globals():
        LOG(f"[{follower_name}] MT5 init failed; skipping this follower.", logging.ERROR)
        return

    # 4) On first run for this follower, relink any existing copier positions
    if not st["links_loaded"]:
        # ensure reverse symbol map is up to date
        global reverse_symbol_map
        reverse_symbol_map = {v: k for k, v in SYMBOL_MAP.items()}
        load_links_from_mt5()
        st["links_loaded"] = True

    # 5) Daily state (per follower, using follower-specific STATE_PATH)
    _ensure_daily_state()

    # 6) Optional one-time preview per follower
    if SHOW_PREVIEW_TABLE and not preview_printed:
        print(f"\n===== Preview for follower: {follower_name} =====")
        print_preview_table(master_snap)
        preview_printed = True

    # 7) Apply master → follower changes (open/close/scale/stops)
    process_master_changes(master_snap)

    # 8) Save runtime back to FOLLOWER_STATE
    st["TRADING_HALTED"] = TRADING_HALTED
    st["SESSION_DAY_KEY"] = SESSION_DAY_KEY
    st["DAY_START_EQUITY"] = DAY_START_EQUITY
    st["preview_printed"] = preview_printed
    st["link_map"] = link_map

def poll_loop():
    """
    Multi-follower main loop:

    1) Poll SiRiX ONCE per interval -> master snapshot.
    2) For each follower in FOLLOWERS:
       - Swap in its MT5 + risk config.
       - Attach to its MT5 terminal.
       - Apply process_master_changes(master_snapshot).
    """
    global reverse_symbol_map
    reverse_symbol_map = {v: k for k, v in SYMBOL_MAP.items()}
    ...
    LOG(f"[init] Multi-follower copier starting with {len(FOLLOWERS)} follower accounts.")
    LOG(f"[mode] VOLUME_MODE={VOLUME_MODE} (risk-multiple), PNL_PARITY_MODE={PNL_PARITY_MODE}")
    LOG("[init] Copier running. Ctrl+C to stop.")

    while running:
        # 1) Fetch master state once
        data = fetch_userstatus_payload(
            SIRIX_MASTER_USER_ID,
            get_open=True,
            get_pending=False,
        ...
        # Update master's equity & open positions snapshot
        update_master_equity(data)
        master_snap = build_master_snapshot(data)

        # 2) Apply to EACH follower
        for name, cfg in FOLLOWERS.items():
            try:
                apply_master_to_follower(name, cfg, master_snap)
            except Exception as e:
                LOG(f"[{name}] ERROR applying master snapshot: {e}", logging.ERROR)

        time.sleep(POLL_INTERVAL_SEC)

    # Clean shutdown when loop exits
    shutdown_mt5()
    LOG("[done] Multi-follower copier stopped.")
