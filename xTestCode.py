...
- 10× master % risk sizing (with cap) — NOT lots multiplier.
...
---------------------------------------------------
Risk = 10× Master Risk %  + Reference-Move Fallback (no SL)
---------------------------------------------------

• Sizing: follower risks K × (master's % risk) per trade, K=RISK_MULTIPLIER (default 10.0).
• If master has SL: use master SL distance.
• If master has NO SL: use a *reference move* per symbol (DEFAULT_STOP_DISTANCE_PRICE) to infer % sensitivity,
  optionally place a *protective SL* at that distance until the master sets a real SL.
• Master % risk is approximated using follower tick_value/tick_size on the mapped follower symbol.
• Optional MAX_RISK_PCT_PER_TRADE cap clamps extreme sizes.
...
MT5_LOGIN: int = 52513858
MT5_PASSWORD: str = "NN6deVgJF&Rzgn"
...
# Protect when master has no SL: put a temporary SL at the reference move (recommended)
PROTECTIVE_SL_IF_MASTER_NONE: bool = True

# Max follower risk per trade (cap after K×). 0.05 = 5% (tune to taste). Set 0 to disable cap.
MAX_RISK_PCT_PER_TRADE: float = 0.15
...
FOLLOWER_UNITS_PER_LOT = {
    # (intentionally unused for risk sizing)
}

# ---- Sizing rule: K × (master % risk) ----
VOLUME_MODE: str = "risk_master_multiple"  # <<<< switched from "multiplier" to risk mode
...
# Default SL distance / reference move (price units) used when master has no SL
# Also used as the "reverse mechanism" ΔP_ref for % sensitivity inference.
DEFAULT_STOP_DISTANCE_PRICE = {
    "XAUUSD":  10.0,   # $10
    "XAGUSD": 0.50,    # $0.50
    "USTEC": 50.0,  # index pts
    "DE40": 50.0,
    "US30":  100.0,
    "US500": 25.0,
...
def _order_send_with_fallback(request: dict, symbol: str):
    ...
    for mode in seq:
        ...
        try:
            ...
            LOG(f"[trade] order_send exception (mode={mode}, {symbol}): {e}", logging.ERROR)
            res = None

        if res is None:
            le = mt5.last_error()
            LOG(f"[trade] order_send returned None (mode={mode}, {symbol}). last_error={le}", logging.ERROR)
            last = None
            continue

        last = res
        r = getattr(res, "retcode", None)
        if r == 10030:  # unsupported filling -> try next
            LOG(f"[filling] {symbol} rejected mode={mode}; trying next...", logging.WARNING)
            continue

        # success or other definitive response
        if r in (mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED, mt5.TRADE_RETCODE_DONE_PARTIAL):
...
def _money_loss_per_lot(symbol: str, stop_distance_price: float) -> Optional[float]:
    ...
    Makes sure the symbol is selected (so tick specs are populated).
    """
    ...
    # Ensure symbol is visible/selected so MT5 populates info fields
    info = mt5.symbol_info(symbol)
    if not DRY_RUN and (info is None or not getattr(info, "visible", False)):
        mt5_symbol_prepare(symbol)
        info = mt5.symbol_info(symbol)

    if info is None:
        if DRY_RUN:
            # harmless dummy if we can't attach in dry-run
            return max(stop_distance_price, 0.0) * 1.0
        LOG(f"[RISK] symbol_info() is None for {symbol}; cannot evaluate tick value.", logging.WARNING)
        return None
    ...
    if tick_value <= 0 or tick_size <= 0:
        LOG(f"[RISK] tick_value/tick_size unavailable for {symbol} (tick_value={tick_value}, tick_size={tick_size}).", logging.WARNING)
        return None
...
def _master_amount_to_lots_raw(master_symbol: str, master_amount: float) -> float:
    """Convert SiRiX 'Amount' to master LOTS (independent of follower sizing)."""
...
def _stop_distance_for(mp: MasterPosition, mapped_symbol: str) -> float:
    """Master SL distance if present, else the reference move (DEFAULT_STOP_DISTANCE_PRICE)."""
    try:
        ...
        pass
    # Fallback to default per symbol (acts as ΔP_ref)
    return DEFAULT_STOP_DISTANCE_PRICE.get(mapped_symbol, DEFAULT_STOP_DISTANCE_PRICE.get(mp.symbol, 10.0))

def _risk_calc(mp: MasterPosition) -> Tuple[float, Dict[str, float]]:
    """
    Compute follower lots so follower_risk% = K × master_risk% (capped).
    If master has no SL, we use the reference move distance as a proxy.
    Returns (lots, debug_info).
    """
    ...
    if not money_per_lot or money_per_lot <= 0:
        ...
        return FIXED_LOTS, {"reason": "tick_missing"}
    ...
    if master_lots <= 0:
        ...
        return FIXED_LOTS, {"reason": "master_lots<=0"}
    ...
    if not master_eq or master_eq <= 0:
        ...
        return 0.0, {"reason": "master_eq_unknown"}

    # Master % risk (approx, using follower tick metrics for mapped symbol)
    master_risk_cash = master_lots * money_per_lot
    master_risk_pct  = master_risk_cash / float(master_eq)

    # Follower target % risk
    follower_eq = account_equity()
    if not follower_eq or follower_eq <= 0:
        return 0.0, {"reason": "follower_eq_unknown"}

    follower_risk_pct  = RISK_MULTIPLIER * master_risk_pct
    if MAX_RISK_PCT_PER_TRADE and MAX_RISK_PCT_PER_TRADE > 0:
        if follower_risk_pct > MAX_RISK_PCT_PER_TRADE:
            LOG(f"[RISK] Cap applied: target {follower_risk_pct*100:.2f}% -> {MAX_RISK_PCT_PER_TRADE*100:.2f}%")
        follower_risk_pct = min(follower_risk_pct, MAX_RISK_PCT_PER_TRADE)
    ...
    dbg = {
        ...
        "master_eq": float(master_eq),
        "master_risk_pct": master_risk_pct,
        "follower_eq": float(follower_eq),
        "follower_risk_pct": follower_risk_pct,
        "follower_risk_cash": follower_risk_cash,
        "lots_raw": lots
    }
    return lots, dbg
...
def _risk_multiple_of_master_lots(mp: MasterPosition) -> float:
    """
    Follower lots = K × (master % risk), applied to follower equity, using follower tick specs.
    - If master has SL, use that distance.
    - If not, use DEFAULT_STOP_DISTANCE_PRICE[mapped_symbol] (reference move / protective assumption).
    - If tick specs are unavailable, fall back to FIXED_LOTS.
    """
    mapped_symbol = map_symbol(mp.symbol)
    if mapped_symbol is None:
        return 0.0

    # Make sure MT5 has symbol specs when not in dry-run
    if not DRY_RUN:
        mt5_symbol_prepare(mapped_symbol)

    # Distance used for risk calc (master SL if present; else reference move)
    stop_distance = _stop_distance_for(mp, mapped_symbol)

    # Master lots derived from master's Amount (per your SYMBOL_UNITS_PER_LOT)
    master_lots = _master_amount_to_lots_raw(mp.symbol, mp.amount)

    # $ loss for 1.0 lot at that distance (on follower broker)
    money_per_lot = _money_loss_per_lot(mapped_symbol, stop_distance)
    if not money_per_lot or money_per_lot <= 0:
        LOG(f"[RISK] Cannot evaluate tick value for {mapped_symbol}; falling back to FIXED_LOTS={FIXED_LOTS}", logging.WARNING)
        LOG(f"[RISK-dbg] {mapped_symbol} Amt={mp.amount:.6f}  mLots={master_lots:.6f}  stopΔ={stop_distance:.6f}", logging.INFO)
        return FIXED_LOTS

    # Master equity must be known to compute master % risk
    master_eq = last_sirix_equity
    if not master_eq or master_eq <= 0:
        LOG("[RISK] Master equity unknown; will wait for next poll to size accurately.", logging.WARNING)
        return 0.0
    ...
    follower_eq = account_equity()
    if not follower_eq or follower_eq <= 0:
        LOG("[RISK] Follower equity unavailable; skipping sizing this tick.", logging.WARNING)
        return 0.0

    target_pct = RISK_MULTIPLIER * master_risk_pct

    # Optional cap if your build defines MAX_RISK_PCT_PER_TRADE
    cap = globals().get("MAX_RISK_PCT_PER_TRADE", 0.0)
    if cap and cap > 0 and target_pct > cap:
        LOG(f"[RISK] Cap applied: target {target_pct*100:.2f}% -> {cap*100:.2f}%", logging.INFO)
        target_pct = cap

    follower_risk_cash = follower_eq * target_pct
    lots_raw = follower_risk_cash / money_per_lot

    LOG(
        "[RISK] %s Amt=%.6f  mLots=%.6f  stopΔ=%.6f  $/lot@Δ=%.4f  m%%=%.4f  f%%=%.4f  f$=%.2f  lots_raw=%.4f"
        % (mapped_symbol, mp.amount, master_lots, stop_distance, money_per_lot,
           master_risk_pct*100.0, target_pct*100.0, follower_risk_cash, lots_raw),
        logging.INFO
    )

    return max(lots_raw, 0.0)
...
def mt5_open_trade(master_pos: MasterPosition, lots: float,
                   sl: Optional[float], tp: Optional[float],
                   comment_suffix: str = "") -> Optional[int]:
    ...
    req = {
        ...
        "magic": MAGIC_NUMBER,
        "comment": f"Copy SiRiX {master_pos.order_number}{(' ' + comment_suffix) if comment_suffix else ''}",
    ...
    if not res or getattr(res, "retcode", None) not in ok_codes:
        LOG(f"[ERR] MT5 open failed ({symbol}): retcode={getattr(res, 'retcode', None)} last_error={mt5.last_error()} details={res}",
            logging.ERROR)
        return None
...
def mt5_adjust_volume(link: FollowerLink, new_lots: float) -> bool:
    ...
    res = _order_send_with_fallback(req, symbol)
    ok_codes = (mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_DONE_PARTIAL, mt5.TRADE_RETCODE_PLACED)
    if not res or getattr(res, "retcode", None) not in ok_codes:
        LOG(f"[ERR] Volume adjust failed ticket {link.follower_ticket}: retcode={getattr(res, 'retcode', None)} last_error={mt5.last_error()}",
            logging.ERROR)
        return False
...
def mt5_update_stops(link: FollowerLink, sl: Optional[float], tp: Optional[float]) -> bool:
    ...
    ok_codes = (mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED, mt5.TRADE_RETCODE_DONE_PARTIAL)
    if not res or getattr(res, "retcode", None) not in ok_codes:
        LOG(f"[ERR] SL/TP update failed ticket {ticket}: retcode={getattr(res, 'retcode', None)} last_error={mt5.last_error()}",
            logging.ERROR)
        return False
...
def print_preview_table(master_snap: Dict[Any, MasterPosition]) -> None:
    ...
    for oid, mp in master_snap.items():
        ...
        # For risk mode, a precise preview requires equity + tick data; keep your original preview simple.
...
def _compute_protective_sl_if_needed(mp: MasterPosition, mapped_symbol: str) -> Tuple[Optional[float], bool]:
    """
    If master has no SL and protective mode is ON, compute a provisional SL at the reference move distance.
    Returns (sl_price_or_None, was_protective_bool).
    """
    has_master_sl = False
    try:
        if mp.stop_loss is not None and mp.open_rate:
            if abs(float(mp.open_rate) - float(mp.stop_loss)) > 0:
                has_master_sl = True
    except Exception:
        pass

    if has_master_sl or not PROTECTIVE_SL_IF_MASTER_NONE:
        return mp.stop_loss, False

    # master has NO SL -> place protective SL at DEFAULT_STOP_DISTANCE_PRICE[mapped]
    bid, ask = mt5_current_prices(mapped_symbol)
    if bid is None or ask is None:
        return None, False
    entry = ask if sirix_is_buy(mp.side) else bid
    ref = DEFAULT_STOP_DISTANCE_PRICE.get(mapped_symbol, DEFAULT_STOP_DISTANCE_PRICE.get(mp.symbol, None))
    if not ref or ref <= 0:
        return None, False
    sl = entry - ref if sirix_is_buy(mp.side) else entry + ref
    return sl, True
...
def open_follower_for_master(mp: MasterPosition) -> None:
    ...
    # Compute protective SL if needed (only when master has no SL)
    sl_for_open, protective = _compute_protective_sl_if_needed(mp, mapped_symbol)
    tp_for_open = mp.take_profit

    comment_suffix = "[protSL]" if protective else ""
    if protective:
        LOG(f"[protSL] {mapped_symbol} using reference move until master SL arrives.")

    ticket = mt5_open_trade(mp, lots, sl_for_open, tp_for_open, comment_suffix=comment_suffix)
    if ticket is None:
    ...
    link = FollowerLink(
        ...
        side=mp.side,
        sl=sl_for_open if sl_for_open is not None else mp.stop_loss,
        tp=tp_for_open,
    )
...
def update_follower_stops_for_master(mp: MasterPosition) -> None:
    # Update stops on follower to match master (or keep protective until master SL exists).
    link = link_map.get(mp.order_number)
    if not link:
...
def process_master_changes(new_snap: Dict[Any, MasterPosition]) -> None:
    ...
    for order_number, mp in new_snap.items():
        ...
        if order_number not in master_open_snapshot:
            ...
            amount_changed = abs((prev.amount or 0) - (mp.amount or 0)) > 1e-8
            stops_changed_now = COPY_STOPS and (stops_changed(prev.stop_loss, mp.stop_loss) or
                                                stops_changed(prev.take_profit, mp.take_profit))
            # In risk mode, SL distance affects % risk, so rescale on SL change too.
            if amount_changed or (VOLUME_MODE == "risk_master_multiple" and stops_changed_now):
                adjust_follower_for_master(mp)
            if stops_changed_now:
...
def _debug_print_risk_preview_once():
    """One-time: for current master snapshot, print sizing math (mLots, stop distance, 10× %)."""
    data = fetch_userstatus_payload(
        SIRIX_MASTER_USER_ID,
        get_open=True, get_pending=False, get_closed=True, get_monetary=False,
        token=SIRIX_TOKEN,
    ...
    for oid, mp in snap.items():
        ...
        stop_d = _stop_distance_for(mp, mapped)  # SL distance or reference move
        ...
        if MAX_RISK_PCT_PER_TRADE and MAX_RISK_PCT_PER_TRADE > 0:
            f_risk_pct = min(f_risk_pct, MAX_RISK_PCT_PER_TRADE)
