...
SIRIX_MASTER_USER_ID: str = "263269"    # 263269
...
MT5_LOGIN: int =  887580
MT5_PASSWORD: str = "%yM65(!JNj"
MT5_SERVER: str = "BlueberryMarkets-Demo02"
...
MT5_TERMINAL_PATH: str = r"C:\MT5\TradeCopier-263269\FUND_10K\terminal64.exe"
...
DEFAULT_STOP_DISTANCE_PRICE = {
    "XAUUSD.pi":  10.0,   # $10
    "XAGUSD.pi": 0.50,    # $0.50
    "NAS100.p": 50.0,  # index pts
    "GER40.p": 50.0,
    "DJ30.p":  100.0,
    "SP500.p": 25.0,
    "EURUSD.pi": 0.0020,  # 20 pips
    "GBPUSD.pi": 0.0020,
    "USDCAD.pi": 0.0020,
    "USDJPY.pi": 0.20,    # 20 pips on 2dp quoting
    "BTCUSD.p": 500.0,    # $500 fallback, tweak if needed
}

# ---- Daily loss cap (ONLY cap retained) ----
# MAX DAILY LOSS FOR THIS ACCOUNT = 2% ($200)
# MAX LOSS FOR THIS ACCOUNT = 4% ($400)
DAILY_LOSS_LIMIT_PCT: float = 0.0175   # e.g., 0.10 = 10% of day-start equity
HALT_CLOSE_ALL: bool = True          # close all copier positions on halt

# ---- Files ----
STATE_PATH: str = "copier_state_10k.json"
...
def poll_loop():
    """
    Single-follower main loop:

    1) Init MT5 once for this follower.
    2) Relink any existing copier positions on this account.
    3) Poll SiRiX ONCE per interval -> master snapshot.
    4) Apply master snapshot to this one follower account.
    """
    global reverse_symbol_map, preview_printed

    # Build reverse map for relink logic (MT5 symbol -> SiRiX symbol)
    ...
    # Init MT5 once (no re-init spam)
    if not init_mt5():
        LOG("[init] MT5 init failed; exiting.", logging.ERROR)
        return

    # Relink any existing copier positions on this MT5 account
    load_links_from_mt5()

    # Daily state (equity baseline, loss cap) using this account's STATE_PATH
    _ensure_daily_state()

    LOG(f"[init] Single-follower copier starting for MT5 account {MT5_LOGIN}.")
